...
export default function AdminAddGuitarToOrderBrowsePage({
  params,
}: {
    ...
    (async () => {
      try {
        ...
        const accountSnap = await getDoc(
          doc(db, "accounts", orderData.accountId),
        );
        if (!cancelled && accountSnap.exists()) {
          setAccount({
            id: accountSnap.id,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const data = await fetchDealerGuitars({ accountId: order.accountId });
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-semibold text-white">
              Add guitar to order #{orderId.slice(0, 8).toUpperCase()}
            </h1>
            <p className="mt-1 text-sm text-neutral-500">
              {account
                ? `Pricing for ${account.name} (${account.discountPercent ?? 0}% off RRP). Choose a guitar, set options, then add to this order.`
                : "Choose a guitar, set options, then add to this order."}
            </p>
          </div>
        </div>
        ...
        {fetching ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((g) => (
              <Link
                key={g.id}
                href={`/admin/orders/${orderId}/add/guitars/${g.id}`}
